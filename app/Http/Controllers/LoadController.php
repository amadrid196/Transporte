...
class LoadController extends Controller
{
    public function index()
    {
        ...
        $page = "loads-index";
        return view("loads.index", compact("multi_select", "page", "show_add_button", "add_button_link", "title_tag", "table_headers", "ajax_data_getting_url", "title", "icon_name"));
    }
    ...
    public function loads_ajax($categoryId = 0)
    {
        ...
        $table_data = Load::with("driver", "shipper", "tractor", "trailer", "consignee", 'broker', 'messageRecords', 'accessories');        //        dd($table_data);
        switch ($categoryId) {
            case $ACTIVE_LOADS:

                $table_data = $table_data->where('status', '!=', 'Billed')
                ...
                break;
            case $PLANNING_LOADS:
                $table_data = $table_data->where('status', 'Pending')
            ...
            case $ALL_LOADS:
                ...
                break;
            case $MY_LOADS:
                $table_data = $table_data->where('user_id', Auth::user()->id)
                    ->where('status', '!=', 'Cancelled')
                    ->get();
                break;
        ...
        $data["columns"] = [
                 ...
                 if(row.shipper){
                   var shippers=\"\";
                    row.shipper.forEach(function (currentValue, index, arr) {
                         ...
                         {
                             shippers=shippers+(index+1)+'. '+current_address+'<br>'
                         }
            ...
            array(
                ...
                var date = \"\"
                  if(row.shipper.length>0)
                  {
                    ...
                    row.shipper.forEach(function (currentValue, index, arr) {
                        if(currentValue.assign)
                        {
                            date += \"To be assigned\"+'<br>'
                        }else
                        {
                ...
                var consignee=\"\";
                    row.consignee.forEach(function (currentValue, index, arr) {
                        ...
                        var splited=current_address.split(\",\");
                            ...
                            {
                                consignee=consignee+(index+originIndex)+'. '+current_address+'<br>'
                            }
            ...
            array("data" => "profit", "render" => "
                ...
                {
                    var profit = '$'+ totalincome.toFixed(2).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')
                    profit;
                }
            "),
        ...
        foreach ($table_data as $val) {
            ...
            foreach ($val->accessories as $currentValue) {

                if ($currentValue->value != null && $currentValue->type == "income")
                    $totalIncome += $currentValue->value;
            }
        }
    ...
    public function create()
    {
        ...
        $title = "Create load";
        $icon_name = "fa-list-alt";
        $factor_agent_settings = FactorAgentSetting::first();
        return view("loads.create", compact('title', 'icon_name', "pageTitle", "url", "page",  "factor_agent_settings"));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('completed_date') && !empty($request->completed_date)) {
            $load->completed_date = $request->completed_date;
        } else {
            if ($request->status == "Billed") {
                $load->completed_date = Date('Y-m-d');
            }
        }
        $load->save();
        ...
        foreach ($request->accesorials2 as $key => $value) {
            $accessory = new AccessorialLoad([
                ...
                "type" => "expense",
                "quantity" => isset($request->quantity_a2ccesorial[$key]) ? $request->quantity_a2ccesorial[$key] : (isset($request->accessory_value2[$key]) ? 1 : 0),
        ...
        if (!empty($request->consignee_id) && $request->consignee_id[0] != null) {
            foreach ($request->consignee_id as $key => $value) {
                if (!empty($request->consignee_id[$key]) && !empty($request->consignee_address[$key])) {
        ...
        if (!empty($request->shipper_id) && $request->shipper_id[0] != null) {
            foreach ($request->shipper_id as $key => $value) {
                ...
                if (!empty($request->shipper_id[$key]) && !empty($request->shipper_address[$key])) {
                    $shipper = new Shipper([
    ...
    public function edit($id)
    {
        ...
        return view("loads.create", compact(
            "pageTitle",
            'title',
            'icon_name',
            "url",
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('accesorials')) {
            ...
            $load->accessories()->save($accessory);
        }
        $accessorials2 = [];
        if ($request->has('accesorials2')) {
            foreach ($request->accesorials2 as $key => $value) {
                $accessory = new AccessorialLoad([
                    "accessorial_id" => $request->accesorials2[$key],
                    "type" => "expense",
                ...
                array_push($accessorials2, $accessory);
            }
            $load->accessories()->saveMany($accessorials2);
        } else {
            $accessory = new AccessorialLoad([
                "accessorial_id" => 0,
                'note' =>  null,
        ...
        if ($request->has('accesorials3')) {
            foreach ($request->accesorials3 as $key => $value) {
                if (empty($request->accesorials3[$key]))
                    $accessorial_id = 0;
                else
                    $accessorial_id = $request->accesorials3[$key];
                $deduction = new Deduction([
        ...
        if (!empty($request->consignee_id) && $request->consignee_id[0] != null) {
            ...
            foreach ($request->consignee_id as $key => $value) {
                ...
                //     continue;
                if (!empty($request->consignee_id[$key]) && !empty($request->consignee_address[$key])) {
                    if ($request->consignee_assign[$key] == "true" || $request->consignee_assign[$key] == 1) {
                        $assign = true;
                    } else {
                        $assign = false;
                    }
        ...
        if (!empty($request->shipper_id) && $request->shipper_id[0] != null) {
            ...
            foreach ($request->shipper_id as $key => $value) {
                ...
                //     continue;
                if (!empty($request->shipper_id[$key]) && !empty($request->shipper_address[$key])) {
                    if ($request->shipper_assign[$key] == "true" || $request->shipper_assign[$key] == 1) {
                        $assign = true;
                    } else {
                        $assign = false;
                    }
    ...
    public function swap_payment_status(Request $request)
    {
        $id = $request->id;
        foreach ($id as $val) {
            $load = Load::findOrFail($val);
    ...
    public function change_status_complete(Request $request)
    {
        foreach ($request->ids as $val) {
            $load = Load::where("id", $val)->first();
            if ($load) {
                $load->status = 'Billed';
                $load->save();
            }
        ...
        return response()->json(['status' => 'success', 'message' => __("Status Changed Successfully.")]);
    }

    public function change_status_paid(Request $request)
    {
        foreach ($request->ids as $val) {
            $load = Load::where("id", $val)->first();
            if ($load) {
                $load->status = 'Paid by Customer';
                $load->save();
            }
        }
