        ...
        <tr style=" font-size:13px;">
            ...
            <td style="width:11%;">
                ...
                @foreach($load->accessories as $val)
                    @if($val->type == "expense")
                    {{$val->accessorial? $val->accessorial->title." ":" "}} 
                    @endif
                @endforeach
            </td>
...
<table style="border-collapse: collapse; width: 100%;" border="1">
    ...
    @foreach($loads as $load)
        ...
        @foreach($load->accessories->where("type","expense") as $load_accessory)
            @if($key != 0)
                <tr>
                    <td style="width: 80%;">{{$load_accessory->accessorial&&isset($load_accessory->accessorial->title)? $load_accessory->accessorial->title." ":" "}} {{$load_accessory->note}}</td>
